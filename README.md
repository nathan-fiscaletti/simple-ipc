...
You can customize the log messages that are generated by the library by creating a custom Logger and applying it. By default, error messages are sent to `log.Printf` and debug messages are not printed. You can override each of these loggers respectively using `ipc.SetDebugLogger()` and `ipc.SetErrorLogger()`. 
